...
namespace Tornado14.TrayApp
{
    public partial class Starter : Form
    {
        ...
        public Starter()
        {
            ...
            List<FileInfo> allProfiles = new List<FileInfo>();

            DirectoryInfo profilesFolder = new DirectoryInfo(Path.Combine(Application.StartupPath, "profiles"));
            foreach (FileInfo configurationFile in profilesFolder.GetFiles())
            {
                if (configurationFile.Extension == ".xml")
                {
                    allProfiles.Add(new FileInfo(configurationFile.FullName));
                }
            }

            DirectoryInfo dataFolderProfiles = new DirectoryInfo(Path.Combine(Settings.Default.DataFolder, "TrayStarterProfile"));
            ...
            foreach (FileInfo configurationFile in allProfiles)
            {
                groupCount += 100000;

                if (configurationFile.Extension == ".xml")
                {
                    List<ProfileConfigItem> configList = Utils.DeserializeParameter(configurationFile.FullName);
            ...
            comboBox1.DataSource = configTextList;
        ...
        {
            comboBox1.Text = text.Split('.')[1].Trim();
            comboBox1.Focus();
        ...
        private void comboBox1_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                {
                    process = Process.Start(selectedApp.app, selectedApp.parameter);
                    process.Exited += new EventHandler(process_Exited);
                    process.Disposed += new EventHandler(process_Disposed);
                }

                ListViewItem item = new ListViewItem();
                if (selectedApp.icon != null && selectedApp.icon.Length > 3)
                {
                    if (!iconList.Images.ContainsKey(selectedApp.icon))
                    {
                        FileInfo i1 = new FileInfo(Path.Combine(iconsFolder.FullName, selectedApp.icon));
                        if (i1.Exists)
                        {
                            Image i = Image.FromFile(Path.Combine(iconsFolder.FullName, selectedApp.icon));
                            iconList.Images.Add(selectedApp.icon, i);
                        }
                        else
                        {
                            Image i = Image.FromFile(Path.Combine(iconsFolder.FullName, "_defaulticons\\stern.png"));
                            iconList.Images.Add(selectedApp.icon, i);
                        }
                        item.ImageKey = selectedApp.icon;
                    }
                }
        ...
        private void listView1_ItemSelectionChanged(object sender, ListViewItemSelectionChangedEventArgs e)
        {
            ...
            ProfileConfigItem selectedApp = (ProfileConfigItem)e.Item.Tag;
            Process process;
        ...
        private void panelStarter_Paint(object sender, PaintEventArgs e)
        {
