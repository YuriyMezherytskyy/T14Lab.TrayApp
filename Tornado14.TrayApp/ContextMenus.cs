...
namespace Tornado14.TrayApp
{
  ...
  class ContextMenus
  {
    ...
    private ContextMenuStrip CreateContextMenu(string profileFileName)
    {
      ...
      foreach (var configGroup in cacheGroupAppList)
      {
        ...
        foreach (var configItem in configGroup.Value)
        {
          ...
          if (configItem.icon != null)
          {
            iconFile = new FileInfo(Path.Combine(iconsFolder.FullName, configItem.icon));
          }
          Image icon1 = null;
          if (configItem.icon != null && iconFile != null && iconFile.Exists)
          {
          ...
          {
            try {
              icon1 = Icon.ExtractAssociatedIcon(configItem.app).ToBitmap();
            } catch (Exception ex)
            {
    ...
    private ToolStripDropDown CreateDropdown(List<string> errors, ref int keyCount, ProfileConfigItem configItem)
    {
      ...
      foreach (var subItem in configItem.subItems)
      {
        ...
        else if (subItem.displayName == "startAll")
        {
          ...
          {
            i2 = new FileInfo(Path.Combine(iconsFolder.FullName, subItem.icon));
          }
          Image subImage = null;
          if (subItem.icon != null && i2 != null && i2.Exists)
          {
          ...
          {
            try {
              subImage = Icon.ExtractAssociatedIcon(subItem.app).ToBitmap();
            } catch (Exception ex)
            {
    ...
    private static void StartApp(ProfileConfigItem appConfig)
    {
